...
public partial class ReceivingPurchaseOrderGrid
{
    [Inject] IReceivingHandler ReceivingHandler { get; set; } = default!;
    [Inject] IGridSettingsService GridSettingsService { get; set; } = default!;
    AppDataGrid<ReceivingPurchaseOrderDataGridVM> PurchaseOrderDataGrid { get; set; }
    ...
    async Task<DataGridResultVM<ReceivingPurchaseOrderDataGridVM>> LoadDataAsync(DataGridIntent intent)
    {
        ...
        return DataGridResultVM<ReceivingPurchaseOrderDataGridVM>.New(action.Result.Data ?? [], action.Result.Count);
    }

    void ViewPurchaseOrder(ReceivingPurchaseOrderDataGridVM purchaseOrder) => NavManager.NavigateTo($"/transactions/purchasing/receiving/purchase-order/view?ref={purchaseOrder.Id}", true);
}
